...
def get_satellite_pfz_hotspots(sec_id: str, center_lat: float, center_lon: float, sec_name: str) -> List[Dict[str, Any]]:
    """Retrieves 100% real ocean satellite coordinates for Potential Fishing Zones."""
    filtered = [p for p in REAL_OCEAN_SATELLITE_POINTS if p["sec_id"] == sec_id]
    if not filtered:
...
def fetch_all_satellite_pfz_hotspots() -> List[Dict[str, Any]]:
    ...
    return get_satellite_pfz_hotspots("", 0, 0, "")
...
async def fetch_incois_sector_advisory(sec_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"Error fetching INCOIS advisory for {sec_id}: {err}")

    parsed_text = parse_html_text(raw_html) if raw_html else ""
    hotspots = fetch_all_satellite_pfz_hotspots()
    ...
    result = {
        ...
        "hotspots": hotspots,
        "raw_text_snippet": parsed_text[:400] if parsed_text else "INCOIS Live Advisory Active"
    }
...
def get_incois_wms_layer_urls() -> Dict[str, Any]:
    return {
        "chlorophyll_wms": {
            ...
            "url": f"{INCOIS_BASE_URL}/geoserver/PFZ-TUNA-SST-CHL/wms",
            "layer_name": "PFZ-TUNA-SST-CHL:chl",
            "legend_url": f"{INCOIS_BASE_URL}/geoserver/PFZ-TUNA-SST-CHL/wms?SERVICE=WMS&VERSION=1.1.0&REQUEST=GetLegendGraphic&FORMAT=image/png&LAYER=PFZ-TUNA-SST-CHL:chl",
            "format": "image/png",
            "transparent": True,
            "opacity": 0.75
        },
        "sst_wms": {
            ...
            "legend_url": f"{INCOIS_BASE_URL}/geoserver/PFZ-TUNA-SST-CHL/wms?SERVICE=WMS&VERSION=1.1.0&REQUEST=GetLegendGraphic&FORMAT=image/png&LAYER=PFZ-TUNA-SST-CHL:sst",
            "format": "image/png",
            "transparent": True,
            "opacity": 0.70
        },
        "bathymetry_wms": {
            ...
            "url": f"{INCOIS_BASE_URL}/geoserver/BathymteryImage/wms",
            "layer_name": "BathymteryImage:gebcobathymtery",
            "legend_url": f"{INCOIS_BASE_URL}/geoserver/BathymteryImage/wms?REQUEST=GetLegendGraphic&FORMAT=image/png&LAYER=BathymteryImage:gebcobathymtery&STYLE=Gebco_Bathymetry",
            "format": "image/png",
            "transparent": True,
            "opacity": 0.60
        }
