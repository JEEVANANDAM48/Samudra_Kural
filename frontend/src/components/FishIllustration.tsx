...
export const FishGroup: React.FC<FishContainerProps> = ({
  ...
  const floatY1 = floatAnim1 ? floatAnim1.interpolate({ inputRange: [0, 1], outputRange: [0, -6] }) : 0;
  const floatY2 = floatAnim2 ? floatAnim2.interpolate({ inputRange: [0, 1], outputRange: [0, 6] }) : 0;
  const floatY3 = floatAnim3 ? floatAnim3.interpolate({ inputRange: [0, 1], outputRange: [0, -7] }) : 0;
  const floatY4 = floatAnim4 ? floatAnim4.interpolate({ inputRange: [0, 1], outputRange: [0, 7] }) : 0;
  ...
  const rot2 = animFish2.x.interpolate({
    inputRange: [-width * 1.5, 0],
    outputRange: ['5deg', '0deg'],
  });
  const scale2 = animFish2.x.interpolate({
    inputRange: [-width * 1.5, 0],
    outputRange: [0.86, 1],
  });

  const rot3 = animFish3.x.interpolate({
    inputRange: [0, width * 1.5],
    outputRange: ['0deg', '-11deg'],
  });
  const scale3 = animFish3.x.interpolate({
    inputRange: [0, width * 1.5],
    outputRange: [1, 0.82],
  });

  const rot4 = animFish4.x.interpolate({
    inputRange: [0, width * 1.5],
    outputRange: ['0deg', '8deg'],
  });
  const scale4 = animFish4.x.interpolate({
    inputRange: [0, width * 1.5],
    outputRange: [1, 0.84],
  });
    ...
    <View style={styles.groupContainer}>
      ...
      <Animated.View
        style={[
          ...
          {
            transform: [
              ...animFish2.getTranslateTransform(),
              { translateY: floatY2 },
              { rotate: rot2 },
              { rotate: wiggle2 },
              { scale: scale2 },
      ...
      {/* Fish 3 - Facing RIGHT -> Moves Right-Up (↗️) */}
      <Animated.View
        style={[
          styles.fishWrapper,
          {
            transform: [
              ...animFish3.getTranslateTransform(),
              { translateY: floatY3 },
              { rotate: rot3 },
              { rotate: wiggle3 },
              { scale: scale3 },
      ...
      {/* Fish 4 - Facing RIGHT -> Moves Right-Down (↘️) */}
      <Animated.View
        style={[
          styles.fishWrapper,
          {
            transform: [
              ...animFish4.getTranslateTransform(),
              { translateY: floatY4 },
              { rotate: rot4 },
              { rotate: wiggle4 },
              { scale: scale4 },
